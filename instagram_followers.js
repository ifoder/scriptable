...
function updateFollowerStatistics(account, followers) {
  ...
  followerStatistic = followerStatistic.filter(
    (currentObj) => currentObj.date != date.getTime()
  );
...
async function extractAvatarImage(html) {
  ...
  let avatarURL = html.substring(
    avatarStart + searchString.length,
    html.indexOf('"', avatarStart + searchString.length)
  );
